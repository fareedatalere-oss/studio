...
import { Bell, Home, PlaySquare, Store, User, X, Bot, Download, MessageSquare, ShieldAlert, CheckCircle2 } from 'lucide-react';
 ...
 * UPGRADED: Master Permission Gate & Audio Unlocker for "Force Ringing".
 */
...
export default function DashboardLayout({
  ...
  const requestMasterPermissions = async () => {
    ...
    // 2. Request Native Notifications
    if ('Notification' in window) {
        const res = await Notification.requestPermission();
        setPermissionState(res);
        if (res === 'granted') {
            toast({ title: "Security Engine Active", description: "Native ringing and push alerts enabled." });
        }
    }
  };
  ...
  useEffect(() => {
    if (user?.$id) {
      ...
      const unsubAlerts = client.subscribe([`databases.default.collections.${COLLECTION_ID_NOTIFICATIONS}.documents`], (response: any) => {
          if (response.payload.userId === user.$id) fetchUnreadCounts();
      });
      return () => unsubAlerts();
    }
  ...
  const handleTabClick = (e: React.MouseEvent, key: string) => {
      if (!isTabOn(key)) {
          e.preventDefault();
          toast({ variant: 'destructive', title: "Not Available", description: "Temporarily disabled." });
      }
  };
  ...
  return (
    <div className="flex min-h-screen flex-col font-body bg-background">
      ...
      {/* MASTER PERMISSION BAR: ONLY IF NOT GRANTED */}
        ...
        <div className="bg-orange-500 text-white p-3 flex items-center justify-between shadow-2xl sticky top-0 z-[100] animate-in slide-in-from-top border-b-2 border-orange-600">
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-[8px] font-bold opacity-90 leading-none mt-1">Allow notifications to enable forced ringing</p>
                </div>
            </div>
              ...
              <Button asChild variant="ghost" size="icon" className="relative h-9 w-9">
                <Link href="/dashboard/notifications">
                  <Bell className="h-4 w-4" />
                   {unreadCount > 0 && <Badge variant="destructive" className="absolute -top-0.5 -right-0.5 h-4 w-4 justify-center p-0 rounded-full text-[8px] font-black border-2 border-white">{unreadCount > 9 ? '9+' : unreadCount}</Badge>}
                </Link>
              </Button>
      ...
      {!isImmersive && (
        <footer className="fixed bottom-0 z-40 w-full border-t bg-background/95 backdrop-blur-md md:hidden shadow-[0_-4px_20px_rgba(0,0,0,0.05)] h-14">
