...
import { Search, ChevronUp, Upload, ShoppingCart, Trash2, Phone, Library, MoreVertical, UserPlus, MessageSquare } from 'lucide-react';
...
export default function MarketContentClient({ initialData, initialTab }: { initialData: any, initialTab: string }) {
  ...
  const [apps] = useState(initialData.apps);
  const [products] = useState(initialData.products);
  const [books] = useState(initialData.books);
  const [upworkProfiles] = useState(initialData.upwork);
