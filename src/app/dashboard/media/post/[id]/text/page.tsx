...
import { ArrowLeft, Loader2, Heart, MessageCircle, Calendar } from 'lucide-react';
...
export default function FullTextPostPage() {
    ...
    useEffect(() => {
        ...
        databases.getDocument(DATABASE_ID, COLLECTION_ID_POSTS, postId)
            .then(setPost)
            .catch(() => {
                toast({ variant: 'destructive', title: 'Error', description: 'Could not load post content.' });
    ...
    return (
        <div className="min-h-screen bg-background p-4 pt-12 pb-24 overflow-y-auto">
            ...
            <Card className="max-w-2xl mx-auto rounded-[2.5rem] overflow-hidden border-none shadow-2xl">
                ...
                <CardContent className={cn("p-8 flex items-center justify-center text-center bg-muted/5", post.backgroundColor)}>
                    <div className="w-full max-h-[60vh] overflow-y-auto py-10 scrollbar-hide">
                        <p className={cn("text-2xl font-black leading-snug whitespace-pre-wrap drop-shadow-sm", (post.backgroundColor === 'bg-white' || !post.backgroundColor) ? 'text-black' : 'text-white')}>
                            {post.text}
                        </p>
                    </div>
                </CardContent>
                <CardFooter className="p-8 bg-muted/10 flex items-center justify-around border-t">
                    <div className="flex flex-col items-center gap-1">
                        <div className="p-4 bg-background rounded-2xl shadow-sm"><Heart className="h-6 w-6 text-red-500" /></div>
                        <span className="text-xs font-black uppercase">{post.likes?.length || 0} Likes</span>
                    </div>
                    <div className="flex flex-col items-center gap-1">
                        <div className="p-4 bg-background rounded-2xl shadow-sm"><MessageCircle className="h-6 w-6 text-primary" /></div>
