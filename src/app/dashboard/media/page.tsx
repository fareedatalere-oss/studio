...
const PostCard = ({ post: initialPost, isMuted, onMuteChange }: { post: any; isMuted: boolean; onMuteChange: (muted: boolean) => void; }) => {
  ...
  const handleAudioPlay = () => {
    document.querySelectorAll('audio').forEach(audioEl => {
      if (audioEl !== audioRef.current) audioEl.pause();
    });
  };
  ...
  const handleFollowToggle = async () => {
    ...
    try {
        ...
        const theirProfile = await databases.getDocument(DATABASE_ID, COLLECTION_ID_PROFILES, post.userId);
        const myCurrentFollowing = myProfile.following || [];
        const theirCurrentFollowers = theirProfile.followers || [];
        
        const newMyFollowing = !currentlyFollowing ? [...myCurrentFollowing, post.userId] : myCurrentFollowing.filter((id: string) => id !== post.userId);
        const newTheirFollowers = !currentlyFollowing ? [...theirCurrentFollowers, currentUser.$id] : theirCurrentFollowers.filter((id: string) => id !== currentUser.$id);
  ...
  const onCommentPostedAction = () => { fetchComments(); setCommentCount(prev => prev + 1); };
        ...
        {post.type === 'music' && (
            <div className="flex flex-col items-center justify-center p-8 text-center w-full h-full bg-muted/20">
                <div className="relative h-64 w-64 mb-10 rounded-full overflow-hidden border-8 border-primary animate-spin-slow shadow-2xl">
                    <Image src={post.thumbnailUrl || "https://picsum.photos/seed/music/400/400"} alt="icon" fill className="object-cover" unoptimized />
      ...
      <div className="absolute left-4 top-1/2 -translate-y-1/2 flex flex-col items-start gap-6 p-2 z-30">
            ...
            {currentUser?.$id !== post.userId && (
                <div className="flex flex-col items-center gap-1">
                    <Button variant={isFollowing ? 'secondary' : 'default'} size="icon" className={cn("h-14 w-14 rounded-full shadow-2xl border-2 border-primary/10 transition-all", isFollowing ? 'bg-green-500 text-white' : 'bg-primary text-white')} onClick={handleFollowToggle} disabled={isLoadingFollow || !currentUser}>
                    ...
                    <span className="text-[10px] font-black text-foreground uppercase">{isFollowing ? 'Unfollow' : 'Follow'}</span>
                </div>
            )}
            <div className="flex flex-col items-center gap-1">
       ...
       <div className="absolute right-4 top-1/2 -translate-y-1/2 flex flex-col items-center gap-6 p-2 z-30">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-14 w-14 rounded-full bg-muted/40 hover:bg-muted/60 text-foreground shadow-xl border-2 border-border/10"><Share className="h-8 w-8" /></Button></DropdownMenuTrigger>
              <DropdownMenuContent className="bg-background">
                <DropdownMenuItem onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/dashboard/media/post/${post.$id}`); toast({ title: 'Link Copied!' }); }} className="font-bold uppercase text-xs"><LinkIcon className="mr-2 h-4 w-4" /><span>Copy Link</span></DropdownMenuItem>
                <DropdownMenuItem onClick={() => { window.location.href = `mailto:?subject=Post from ${post.username}&body=${window.location.origin}/dashboard/media/post/${post.$id}`; }} className="font-bold uppercase text-xs"><Mail className="mr-2 h-4 w-4" /><span>Send to Email</span></DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
            {(post.type === 'reels' || post.type === 'film' || post.type === 'music') && <Button variant="ghost" size="icon" className="h-14 w-14 rounded-full bg-muted/40 hover:bg-muted/60 text-foreground shadow-xl border-2 border-border/10" onClick={toggleMute}>{isMuted ? <VolumeX className="h-8 w-8" /> : <Volume2 className="h-8 w-8" />}</Button>}
      ...
      <div className="absolute bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-background/80 via-background/40 to-transparent z-10">
       ...
       <Sheet open={showComments} onOpenChange={setShowComments}>
        <SheetContent side="bottom" className="h-[80vh] flex flex-col text-foreground rounded-t-[2.5rem] border-t-4 border-primary shadow-2xl">
            <SheetHeader><SheetTitle className="text-center font-black uppercase text-sm tracking-[0.2em] border-b pb-6">Comments ({commentCount})</SheetTitle></SheetHeader>
            <div className="flex-1 overflow-y-auto space-y-6 p-6 scrollbar-hide">
                {commentsLoading ? <div className="flex justify-center items-center h-full"><Loader2 className="animate-spin text-primary h-10 w-10" /></div> : comments.length > 0 ? comments.map(comment => <CommentItem key={comment.$id} comment={comment} />) : <p className="text-center text-muted-foreground py-16 italic font-medium">No comments yet. Be the first to share your thoughts!</p>}
            </div>
            <div className="mt-auto p-6 border-t bg-background pb-10 shadow-inner"><CommentInput postId={initialPost.$id} postOwnerId={initialPost.userId} onCommentPosted={onCommentPostedAction} /></div>
        </SheetContent>
      </Sheet>
    </div>
  );
...
export default function MediaPage() {
  ...
  useEffect(() => {
        ...
        databases.listDocuments(DATABASE_ID, COLLECTION_ID_POSTS, [Query.orderDesc('$createdAt'), Query.limit(100)])
            .then(res => setAllPosts(res.documents))
        ...
        const unsubscribe = databases.client.subscribe([`databases.${DATABASE_ID}.collections.${COLLECTION_ID_POSTS}.documents`], response => {
            ...
            if (response.events[0].includes('.create')) setAllPosts(prev => [payload, ...prev.filter(p => p.$id !== payload.$id)]);
            else if (response.events[0].includes('.delete')) setAllPosts(prev => prev.filter(p => p.$id !== payload.$id));
            else if (response.events[0].includes('.update')) setAllPosts(prev => prev.map(p => p.$id === payload.$id ? payload : p));
        });
        return () => unsubscribe();
    }, []);
