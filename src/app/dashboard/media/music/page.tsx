...
import { databases, DATABASE_ID, COLLECTION_ID_POSTS, COLLECTION_ID_PROFILES, COLLECTION_ID_NOTIFICATIONS } from '@/lib/appwrite';
...
export default function MusicLibraryPage() {
    ...
    const handleBack = () => router.push('/dashboard/media');

    return (
        <div className="h-screen bg-background flex flex-col overflow-hidden">
            {/* Header */}
            <header className="p-4 pt-12 bg-background border-b z-30">
                <div className="flex items-center gap-4 mb-4">
                    <Button onClick={handleBack} variant="ghost" size="icon" className="rounded-full bg-muted/50 border">
                        <ArrowLeft className="h-5 w-5" />
                    </Button>
                    <div className="flex-1">
                        <h1 className="font-black uppercase text-sm tracking-widest text-primary">Music Hub</h1>
                    </div>
                </div>
                ...
                <div className="relative mb-4">
                    ...
                    <Input 
                        ...
                        className="pl-10 rounded-full h-10 bg-muted/50 border-none"
                        value={searchQuery}
                    ...
                    <div className="flex gap-2 pb-2">
                        {categories.map((cat) => (
                            <Button
                                ...
                                className={cn(
                                    "rounded-full font-black uppercase text-[10px] tracking-tighter h-8",
                                    selectedCategory === cat ? "bg-primary" : "bg-muted/30 border-none"
                                )}
                            >
                                {cat}
                            </Button>
            ...
            <main className="flex-1 overflow-y-auto snap-y snap-mandatory scrollbar-hide">
                {loading ? (
                    <div className="h-full flex flex-col items-center justify-center">
                        ...
                        <p className="font-black uppercase text-xs text-muted-foreground animate-pulse">Tuning In...</p>
                    </div>
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-muted-foreground p-10 text-center">
                        <Music className="h-16 w-16 opacity-20 mb-4" />
                        <p className="font-black uppercase text-sm tracking-widest">No tracks found in this category</p>
                    </div>
                )}
            </main>
        </div>
...
function MusicPostCard({ post, isMuted, onMuteChange, currentUser, currentUserProfile, recheckUser }: any) {
    ...
    const handleFollow = async () => {
        ...
        try {
            ...
            const newFollowing = newState ? [...(myProfile.following || []), post.userId] : (myProfile.following || []).filter((id: string) => id !== post.userId);
            const newFollowers = newState ? [...(theirProfile.followers || []), currentUser.$id] : (theirProfile.followers || []).filter((id: string) => id !== currentUser.$id);
    ...
    return (
        <div ref={postRef} className="relative h-screen w-full flex flex-col justify-center snap-start shrink-0 overflow-hidden bg-background">
            <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center bg-muted/10">
            ...
            <div className="absolute left-4 top-1/2 -translate-y-1/2 flex flex-col items-start gap-6 p-2 z-20">
                <div className="flex flex-col items-center gap-2">
                    <Avatar className="ring-2 ring-primary h-14 w-14 shadow-xl">
                        ...
                        <AvatarFallback className="bg-primary text-white font-black">{post.username?.charAt(0)}</AvatarFallback>
                    </Avatar>
                ...
                <div className="flex flex-col items-center gap-1">
                    <Button variant="ghost" size="icon" className="h-14 w-14 rounded-full bg-muted/40 shadow-xl border" onClick={handleLike}>
            ...
            <div className="absolute right-4 top-1/2 -translate-y-1/2 flex flex-col items-center gap-6 p-2 z-20">
                <Button variant="ghost" size="icon" className="h-14 w-14 rounded-full bg-muted/40 shadow-xl border">
                    <MessageCircle className="h-8 w-8" />
                </Button>
                <Button variant="ghost" size="icon" className="h-14 w-14 rounded-full bg-muted/40 shadow-xl border" onClick={() => onMuteChange(!isMuted)}>
            ...
            <div className="absolute bottom-8 left-0 right-0 px-8 text-center">
                <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">{formatDistanceToNow(new Date(post.$createdAt), { addSuffix: true })}</p>
            </div>
        </div>
    );
}
