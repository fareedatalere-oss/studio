            ...
            <Link href={otherUser ? `/dashboard/chat/${otherUser.$id}` : '#'} className="flex-1 flex items-center gap-3 overflow-hidden">
                ...
                <div className="flex-1 overflow-hidden">
                    <div className="flex justify-between items-center mb-0.5">
                        <p className="font-bold text-xs">@{otherUser?.username || 'loading...'}</p>
                    ...
                    <div className="flex justify-between items-center gap-2">
                        ...
                        {unreadCount > 0 && (
                            <Badge variant="destructive" className="h-5 min-w-5 p-0 px-1 text-[10px] font-black rounded-full border-2 border-white flex items-center justify-center">
                                {unreadCount}
                            </Badge>
                        )}
                    </div>
            ...
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-48 font-black uppercase text-[9px]">
                    <DropdownMenuItem onClick={async () => {
                        ...
                        <Trash2 className="mr-2 h-3.5 w-3.5" /> Delete History
                    </DropdownMenuItem>
                </DropdownMenuContent>
            </DropdownMenu>
        </div>
...
export default function ChatPage() {
    ...
    useEffect(() => {
        ...
        const unsubChats = onSnapshot(q, (snapshot) => {
            ...
            data.sort((a: any, b: any) => safeGetTime(b?.lastMessageAt) - safeGetTime(a?.lastMessageAt));
            setRecentChats(data);
            setLoading(false);
        }, () => setLoading(false));
            ...
            <header className="p-4 pt-12 max-w-xl mx-auto w-full border-b bg-muted/5">
                ...
                <Tabs defaultValue="recent" className="w-full">
                    <TabsList className="grid grid-cols-2 bg-muted/50 h-12 rounded-2xl p-1 mb-6 border">
                        ...
                        <TabsTrigger value="all" className="rounded-xl font-black uppercase text-[10px] tracking-widest">All Users</TabsTrigger>
                    </TabsList>
